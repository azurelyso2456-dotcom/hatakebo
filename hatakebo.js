...
function seasonalVegetables(list, month) {
  ...
  return list.map(function(v,i){return {v:v,i:i};}).sort(function(a,b){return rank(a.v)-rank(b.v)||a.i-b.i;}).map(function(e){return e.v;});
}
...
function HatakeApp() {
  ...
  function exportData() {
    ...
    try { localStorage.setItem("hatakebo_lastBackup", new Date().toISOString().slice(0,10)); } catch(e){}
  }
  ...
  /* ── 文字サイズ（標準・大きめ・特大） ── */
  const [uiScale, setUiScale] = useState(function(){
    try { return Number(localStorage.getItem("hatakebo_uiScale")) || 1.0; } catch(e){ return 1.0; }
  });
  ...
  return (
    <div style={{zoom:uiScale}}>
      ...
      <FarmMap farms={farms} plantings={plantings} setPlantings={setPlantings} ridges={ridges} setRidges={setRidges}
        initialFarmId={activeFarmId} onActiveFarmChange={setActiveFarmId}
...
function landmarkIndex(item) {
  var i=LM_TYPES.findIndex(function(t){return item&&(t.id===item.id||t.icon===item.icon);});return i<0?5:i;
}
function LandmarkImage({item,size,x,y}) {
  var i=landmarkIndex(item), n=size||26;
  return <svg x={x===undefined?undefined:x-n/2} y={y===undefined?undefined:y-n/2} width={n} height={n} viewBox={(i%3)+" "+Math.floor(i/3)+" 1 1"} role="img" aria-label={item&&(item.memo||item.label)||"目印"} style={{display:"inline-block",verticalAlign:"middle",pointerEvents:"none",overflow:"hidden",flexShrink:0}}><image href={window.HATAKEBO_LANDMARK_ATLAS} width="3" height="2" preserveAspectRatio="none"/></svg>;
}
function landmarkMarkup(item,size,x,y) {
  var i=landmarkIndex(item);return '<svg x="'+(x-size/2)+'" y="'+(y-size/2)+'" width="'+size+'" height="'+size+'" viewBox="'+(i%3)+' '+Math.floor(i/3)+' 1 1" style="overflow:hidden"><image href="'+window.HATAKEBO_LANDMARK_ATLAS+'" width="3" height="2" preserveAspectRatio="none"/></svg>';
}
...
/* ══════════════════════════════════════
   はじめてガイド（3枚カード）
══════════════════════════════════════ */
function WelcomeGuide({ onClose, isTouchDevice }) {
  ...
  var card = CARDS[page];
  var isLast = page === CARDS.length - 1;
  return (
    <div style={{position:"fixed",inset:0,zIndex:100,background:"rgba(28,20,8,0.55)",
      ...
      <div style={{background:C.paper,border:"2px solid "+C.ink,boxShadow:"6px 6px 0 rgba(28,20,8,0.3)",
        ...
        <div style={{fontSize:19,fontFamily:SERIF,letterSpacing:3,color:C.ink,marginBottom:14}}>{card.title}</div>
        <div style={{fontSize:14,color:C.inkFaint,fontFamily:HAND,lineHeight:2,letterSpacing:0.5,marginBottom:28,textAlign:"left"}}>{card.body}</div>
        <div style={{display:"flex",gap:6,justifyContent:"center",marginBottom:24}}>
          {CARDS.map(function(_,i){
            return <div key={i} style={{width:10,height:10,borderRadius:"50%",
              background:i===page?C.ink:C.inkLine}}/>;
          })}
        </div>
        <button
          onClick={function(){ if(isLast) onClose(); else setPage(page+1); }}
          style={{width:"100%",padding:"16px 0",border:"2px solid "+C.ink,background:C.ink,color:C.paper,
            fontSize:16,cursor:"pointer",fontFamily:SERIF,letterSpacing:3,
            boxShadow:"3px 3px 0 "+C.inkFaint,marginBottom:10}}>
          {isLast ? "はじめる" : "つぎへ"}
        </button>
        {!isLast && (
...
function FarmSetup({ farms, onComplete, onSkip }) {
  ...
  function applyPreset(p) {
    ...
    setWidthM(c*CELL_CM/100); setHeightM(r*CELL_CM/100);
    var fn = p.cells || function(){ return true; };
    setGrid(makeGrid(r, c, fn));
    setStep(2);
  }
  function done() {
    onComplete({ id:"farm_"+Date.now(), name:name||("畑"+(farms.length+1)), rows:rows, cols:cols, grid:grid, cellCm:CELL_CM, widthM:cols*CELL_CM/100, heightM:rows*CELL_CM/100, landmarks:landmarks });
  }
          ...
          <div>
            ...
            {(function(){
              function MStep({ label, value, onChange }) {
                var steps = [];
                for (var v = 0.5; v <= 15.0; v = Math.round((v+0.25)*100)/100) steps.push(v);
  ...
  return (
    <div style={{transition:"transform 0.12s ease-out", transform:"scale("+zoom+")", transformOrigin:"50% 30%", display:"inline-block"}}>
        ...
        return row.map(function(active,c){
          ...
          return (
            <g key={"lmI"+r+"-"+c} style={{pointerEvents:"none"}}>
              <LandmarkImage item={item} x={cx2} y={cy2-1} size={fs}/>
      ...
      {padT > 0 && Array.from({length:farm.cols}, function(_,c){
        ...
        return (
          <g key={"lmT"+c}>
            <LandmarkImage item={item} x={cx2} y={cy2-2} size={LM_FS}/>
...
function PickerRow({ label, value, min, max, onChange, unit }) {
  return (
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,paddingBottom:14,borderBottom:"1px solid "+C.inkLine}}>
      ...
      <button onClick={function(){ if(value<max) onChange(value+1); }}
        style={{width:52,height:52,border:"1px solid "+C.inkBorder,background:C.paper,
          fontSize:24,color:value<max?C.ink:C.inkLine,cursor:value<max?"pointer":"default",
          display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontFamily:SERIF}}>＋</button>
    </div>
  );
}
function ridgePlacementError(farm, ridge, existing) {
  var b = ridgeBBox(ridge), eps = 0.000001;
  if (b.xa < -eps || b.ya < -eps || b.xb > farm.cols+eps || b.yb > farm.rows+eps) return "畑の外にはみ出しています。位置か畝幅を調整してください";
  for (var r=Math.floor(b.ya+eps);r<Math.ceil(b.yb-eps);r++) for(var c=Math.floor(b.xa+eps);c<Math.ceil(b.xb-eps);c++) {
    if(!farm.grid[r] || !farm.grid[r][c]) return "畑ではない場所に重なっています";
  }
...
function plannedRidges(farm, ori, width, count, lengthM, pathCm, edgeCm) {
  ...
  if(!Number.isInteger(count)||count<1||count>capacity) return [];
  var used=count*width+(count-1)*gap, start=(cross-used)/2+width/2;
...
function RidgePicker({ farm, farmRidges, year, pickerOri, setPickerOri, pickerA, setPickerA, pickerStart, setPickerStart, pickerEnd, setPickerEnd, onConfirm, onCancel, isInvalid, pickerVid, setPickerVid, pickerRidgeW, setPickerRidgeW, onBulkConfirm }) {
  ...
  /* 実効 ridgeW */
  var effectiveRW = pickerRidgeW || 60/cellCm;

  /* ミニプレビューSVG（最小12px保証・大きな畑はスクロール） */
  var fitCS = Math.floor((typeof window!=="undefined"?Math.min(window.innerWidth-64,340):300) / Math.max(farm.rows, farm.cols));
  var miniCS = Math.max(12, Math.min(48, fitCS));
  var needScroll = fitCS < 12; /* 全体が収まらない場合 */
  var W = farm.cols * miniCS, H = farm.rows * miniCS;
  var svgTapRef = React.useRef(null);

  /* ① プレビュー直接タップで位置決め */
  function onPreviewTap(e) {
    var svg = svgTapRef.current;
    if (!svg) return;
    var rect = svg.getBoundingClientRect();
    var px = (e.clientX - rect.left) / rect.width  * farm.cols;
    var py = (e.clientY - rect.top)  / rect.height * farm.rows;
    if (isH) {
      var row = Math.min(farm.rows, Math.max(1, Math.round(py + 0.5)));
      setPickerA(row);
    } else {
      var col = Math.min(farm.cols, Math.max(1, Math.round(px + 0.5)));
      setPickerA(col);
    }
  }
      ...
      <div style={{padding:"16px 16px 32px"}}>
            ...
            {[{id:"H",label:"━ 横畝（左右に長い）"},{id:"V",label:"┃ 縦畝（上下に長い）"}].map(function(o){
              ...
              return (
                <button key={o.id}
                  onClick={function(){
                    setPickerOri(o.id);
                    setPickerA(o.id==="H" ? Math.ceil(farm.rows/2) : Math.ceil(farm.cols/2));
                    setPickerStart(1);
                    setPickerEnd(o.id==="H" ? farm.cols : farm.rows);
                  }}
        ...
        {creationMode==="batch" && onBulkConfirm && Object.keys(farmRidges).length === 0 && (
          <div style={{marginBottom:20,border:"2px solid "+C.green,background:C.greenPale,padding:"14px 12px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,fontSize:14}}>
              ...
              <label>本数<select aria-label="自動配置の本数" value={batchCount} onChange={function(e){setBatchCount(Number(e.target.value));}} style={{display:"block",width:"100%",minHeight:44,fontSize:16}}>{Array.from({length:15},function(_,i){return <option key={i+1} value={i+1}>{i+1}本</option>;})}</select></label>
        ...
        <div style={{marginBottom:20,display:"flex",justifyContent:"center",flexDirection:"column",alignItems:"center",gap:8}}>
          ...
          <svg ref={svgTapRef} viewBox={"0 0 "+W+" "+H} width={W} height={H}
            ...
            {Array.from({length:farm.rows+1},function(_,i){
              return <line key={"hg"+i} x1={0} y1={i*miniCS} x2={W} y2={i*miniCS} stroke={C.inkLine} strokeWidth={0.5}/>;
            })}
            {farm.grid.flatMap(function(row,r){
              return row.map(function(active,c){
                if(active) return null;
                return <rect key={"ia"+r+"-"+c} x={c*miniCS} y={r*miniCS} width={miniCS} height={miniCS} fill={C.paper} opacity={0.85}/>;
              });
            }).filter(Boolean)}
            {Object.values(farmRidges).map(function(ridge){
            ...
            {previewRect && (
              <rect x={previewRect.x} y={previewRect.y} width={previewRect.w} height={previewRect.h}
                fill={isInvalid?C.redPale:C.indigoPale} stroke={isInvalid?C.red:C.indigo}
                strokeWidth={2} strokeDasharray="5 3" rx={2} opacity={0.9}/>
            )}
          </svg>
          </div>
        ...
        <div style={{marginBottom:20}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {seasonalVegetables(VEGGIES).map(function(v){
              ...
              return (
                <button key={v.id}
                  onClick={function(){ setPickerVid(isSel ? null : v.id); }}
                  style={{
                    padding:"8px 10px",
                  ...
                  <VegetableImage id={v.id} size={24}/>
                  <span>{v.name}</span>
                </button>
              );
            })}
        ...
        {/* 決定ボタン */}
        <button onClick={onConfirm} disabled={isZero||isInvalid}
          style={{width:"100%",padding:"18px 0",
            border:"2px solid "+(isZero||isInvalid?C.inkLine:C.ink),
            background:isZero||isInvalid?C.paper2:C.ink,
            color:isZero||isInvalid?C.inkLine:C.paper,
            fontSize:16,cursor:isZero||isInvalid?"default":"pointer",
...
function FarmMap({ farms, plantings, setPlantings, ridges, setRidges, snapshots, setSnapshots, soil, setSoil, onAddFarm, onDeleteFarm, onRenameFarm, onExport, onImport, onShowFaq, onUpdateFarm, onRestoreImport, onUndo, canUndo, saveError, uiScale, onChangeUiScale, initialFarmId, onActiveFarmChange }) {
  ...
  const showBackupNudge = totalRidgeCount >= 3 && (function(){
    ...
    var daysSince = (Date.now() - new Date(lastBackup).getTime()) / 86400000;
    return daysSince >= 30;
  })();
  ...
  function undoNewRidge(id) {
    setRidges(function(prev){var fd=Object.assign({},prev[fid]||{});delete fd[id];return Object.assign({},prev,{[fid]:fd});});
    setPlantings(function(prev){var f=Object.assign({},prev[fid]||{});if(f[year]){var y=Object.assign({},f[year]);delete y[id];f[year]=y;}return Object.assign({},prev,{[fid]:f});});
    setSelRid(function(current){return current===id?null:current;});
  }
  ...
  function handleFieldTap(pt){
    ...
    const defaultRW = 60/(farm.cellCm||100);
    const nr={id:id,gx:gx,gy:gy,gl:gl,orientation:orientation,name:n+"番畝",addedFrom:year,ridgeW:defaultRW};
    var placementError=ridgePlacementError(farm,nr,farmRidges);
    if(placementError){ showToast(placementError); setS1(null); setHov(null); return; }
    setRidges(function(prev){
      const fd=prev[fid]||{};
      return Object.assign({},prev,{[fid]:Object.assign({},fd,{[id]:nr})});
    });
    setS1(null); setHov(null);
    setSelRid(id); setTab("plant");
    showToast("✓ 畝を引きました",function(){undoNewRidge(id);});
  }
  ...
  function openRidgePicker() {
    ...
    setPickerVid(null);
    setPickerRidgeW(60/(farm.cellCm||100));
    setShowRidgePicker(true);
    closeSheet();
    setS1(null); setHov(null);
  }
  function pickerToRidge() {
    var isH = pickerOri === 'H';
    var sc = pickerStart - 1; /* 0-indexed */
    var ec = pickerEnd;       /* 0-indexed exclusive */
    if (isH) return { orientation:'H', gx:(sc+ec)/2, gy:pickerA-1+0.5, gl:ec-sc };
    return            { orientation:'V', gx:pickerA-1+0.5, gy:(sc+ec)/2, gl:ec-sc };
  }
  /* ── ② となりにもう1本（平行畝を複製） ── */
  function addAdjacentRidge(srcId) {
    var src = (ridges[fid]||{})[srcId];
    if (!src) return;
    var rw = src.ridgeW || RWIDTH;
    var gap = 40 / (farm.cellCm||100); /* 新しく追加する畝は通路40cm */
    var offset = rw + gap;
    var maxPerp = src.orientation === "H" ? farm.rows : farm.cols;

    /* +方向 → だめなら −方向 */
    var candidates = [offset, -offset];
    var nr = null;
    for (var i = 0; i < candidates.length; i++) {
      var perp = (src.orientation === "H" ? src.gy : src.gx) + candidates[i];
      if (perp - rw/2 < 0 || perp + rw/2 > maxPerp) continue;
      var cand = src.orientation === "H"
        ? { orientation:"H", gx:src.gx, gy:perp, gl:src.gl }
        : { orientation:"V", gx:perp, gy:src.gy, gl:src.gl };
      if (ridgeOnInactiveCell(Object.assign({ridgeW:rw}, cand))) continue;
      /* 既存のアクティブ畝と重ならないか */
      var overlaps = Object.values(farmRidges).some(function(r){
        return ridgesOverlap(Object.assign({ridgeW:rw}, cand), r);
      });
      if (overlaps) continue;
      nr = cand;
      break;
    }
    if (!nr) { showToast("⚠ となりに引く場所がありません"); return; }

    var id = 'ridge_' + Date.now();
    var n = Object.keys(farmRidges).length + 1;
    var srcVid = extractVid(plantings[fid] && plantings[fid][year] && plantings[fid][year][srcId]);
    setRidges(function(prev){
      var fd = prev[fid] || {};
      return Object.assign({}, prev, {[fid]: Object.assign({}, fd, {[id]: Object.assign({}, nr, {id:id, name:n+'番畝', addedFrom:year, ridgeW:rw})})});
    });
    if (srcVid) sp(id, srcVid, year, null, null);
    showToast("✓ となりに畝を引きました", function(){
      setRidges(function(prev){
        var fd=Object.assign({},prev[fid]||{}); delete fd[id];
        return Object.assign({},prev,{[fid]:fd});
      });
      if (srcVid) setPlantings(function(prev){
        var f=Object.assign({},prev[fid]||{});
        if(f[year]){ var y=Object.assign({},f[year]); delete y[id]; f[year]=y; }
        return Object.assign({},prev,{[fid]:f});
      });
    }, { label:"となりにもう1本", fn:function(){ addAdjacentRidge(id); } });
  }
  ...
  function bulkCreateRidges(n, ori, lengthM, pathCm, edgeCm) {
    ...
    for (var i = 0; i < n; i++) {
      var cand = plan[i];
      if (ridgeOnInactiveCell(Object.assign({ridgeW:rw}, cand))) continue;
      var id = 'ridge_' + Date.now() + '_' + i;
      created[id] = Object.assign({}, cand, {id:id, name:(Object.keys(farmRidges).length + count + 1)+'番畝', addedFrom:year, ridgeW:rw});
      newIds.push(id);
      count++;
    }
    if (newIds.length === 0) { showToast("⚠ 畝を引ける場所がありません"); return; }
    setRidges(function(prev){
      var fd = Object.assign({}, prev[fid] || {}, created);
      return Object.assign({}, prev, {[fid]: fd});
    });
    setShowRidgePicker(false);
    setPickerVid(null);
    showToast("✓ "+newIds.length+"本の畝を引きました", function(){
      setRidges(function(prev){
        var fd = Object.assign({}, prev[fid] || {});
        newIds.forEach(function(id){ delete fd[id]; });
        return Object.assign({}, prev, {[fid]: fd});
      });
    });
  }

  function confirmRidgePicker() {
    var nr = pickerToRidge();
    nr.ridgeW=pickerRidgeW || 60/(farm.cellCm||100);
    if (nr.gl < 0.5) return;
    var placementError=ridgePlacementError(farm,nr,farmRidges);
    if (placementError) {showToast(placementError);return;}
    var id = 'ridge_' + Date.now();
    var n = Object.keys(farmRidges).length + 1;
    setRidges(function(prev){
      var fd = prev[fid] || {};
    ...
    if (pickerVid) {
      ...
      sp(id, pickerVid, year, null, null);
      setShowRidgePicker(false);
      setPickerVid(null);
      showToast("✓ "+(VM[pickerVid]?VM[pickerVid].name+"の":"")+"畝を引きました", function(){
        setRidges(function(prev){
          const fd=Object.assign({},prev[fid]||{}); delete fd[id];
          return Object.assign({},prev,{[fid]:fd});
        });
        setPlantings(function(prev){
          const f=Object.assign({},prev[fid]||{});
          if(f[year]){ const y=Object.assign({},f[year]); delete y[id]; f[year]=y; }
          return Object.assign({},prev,{[fid]:f});
        });
      }, { label:"となりにもう1本", fn:function(){ addAdjacentRidge(id); } });
    } else {
  ...
  var pickerRidgePreview = showRidgePicker ? pickerToRidge() : null;
  ...
  function delRidge(rid){
    ...
    if (ridge) {
      ...
      const ridgeBefore = ridge; /* deletedFrom なしの状態 */
      ...
      showToast("「"+ridge.name+"」を削除しました", function(){
        ...
        /* 転記した soil レコードを末尾から取り除く */
        if (recCount > 0) setSoil(function(prev){
          const arr = (prev[fid]||[]).slice(0, -recCount);
          return Object.assign({},prev,{[fid]:arr});
        });
      });
    }
    setSelRid(null);
  ...
  return (
    <div style={Object.assign({minHeight:"100vh",background:C.paper,fontFamily:SERIF,color:C.ink,userSelect:"none"},pageLines)}
                ...
                <div style={{padding:"12px 16px",borderBottom:"1px solid "+C.inkLine}}>
                  ...
                  <div style={{display:"flex",gap:5}}>
                    {[{v:1.0,l:"標準"},{v:1.2,l:"大きめ"},{v:1.4,l:"特大"}].map(function(o){
                      var sel = Math.abs((uiScale||1.0)-o.v)<0.01;
                      return (
                        <button key={o.v}
                          onClick={function(e){e.stopPropagation();onChangeUiScale(o.v);}}
                          style={{flex:1,padding:"9px 0",border:"2px solid "+(sel?C.ink:C.inkLine),
                            background:sel?C.ink:"transparent",color:sel?C.paper:C.inkFaint,
                            fontSize:o.v===1.0?12:o.v===1.2?14:16,cursor:"pointer",fontFamily:SERIF}}>
                          {o.l}
                        </button>
                      );
                    })}
      ...
      <div style={{padding:"20px 16px",paddingBottom:(bottomBarH+24)+"px",overflowX:"auto",touchAction:(s1||isPressing)?"none":"auto"}} onClick={function(e){e.stopPropagation();}}>
        ...
        {farm && mainTab === "map" && (
          <div style={{display:"inline-block",minWidth:"fit-content"}}>
            ...
            {/* 畝が0のヒント */}
            {ridgeCount===0 && !s1 && (
              <div style={{marginTop:16,padding:"20px 16px",border:"2px dashed "+C.inkLine,background:C.paper2,textAlign:"center"}}>
      ...
      {showRidgePicker && farm && (
        <RidgePicker
          farm={farm} farmRidges={farmRidges} year={year}
          pickerOri={pickerOri} setPickerOri={setPickerOri}
          pickerA={pickerA} setPickerA={setPickerA}
          ...
          onBulkConfirm={bulkCreateRidges}
    ...
    "items": [
      {
        "q": "PCでもボタンで畝を作れますか？",
        "a": [
          "PC・スマートフォンとも、画面下の「＋ 畝を引く」から作れます。向き・畝幅・位置・始めと終わりを選び、プレビューを確認して「この畝を引く」を押してください。",
          "PCでは畑の上をドラッグする方法も残っています。ドラッグ作成の畝幅は60cmです。幅を選びたいときはボタンから作成してください。"
        ]
      },
      {
        "q": "畑の大きさと畝の幅は、どう決まりますか？",
        "a": [
          "畑は土地全体の大きさ、畝はその中に置く栽培場所です。畑の寸法は25cm刻み、各辺50cm〜15mの範囲で登録します。畝幅は30・60・90・120cmから選べます。",
          "本数を減らしても畝幅は広がりません。10m四方の畑に、幅60cm・長さ3mの畝を2本、間の通路40cmで置くと、横並びの幅は1.6mです。残った土地は空き地になります。"
        ]
      },
      {
        "q": "おまかせで畝をまとめて作るには？",
  ...
  },
  {
    ...
    "items": [
      ...
      },
      {
        "q": "連作の注意表示は何を見ていますか？",
        "a": [
          "登録した作付けと過去2年までの記録を比べ、同じ科が連続しているかを見ています。同じ科が2年続く場合は橙、3年続く場合は赤の注意を表示します。",
          "同じ場所に重なる過去の記録も参照しますが、土の状態や病害虫を診断するものではありません。記録がない年や、登録内容の違いによって結果が変わります。"
        ]
      },
      {
        "q": "自動保存と「記録する」は何が違いますか？",
        "a": [
          "畑や作付けの変更は、その都度ブラウザ内に自動保存されます。「記録する」を押さなくても通常の入力内容は保存されます。",
          "「記録する」は、その時点の畝と作付けをきろく帳に残す操作です。作物を植え替える前後などに使います。前回と同じ状態では「変化なし」となり、重複して記録できません。",
          {
            "type": "note",
            "text": "きろく帳も同じブラウザ内の保存です。端末の故障やサイトデータの消去に備えるには、別途バックアップが必要です。"
          }
        ]
      },
      {
    ...
    "items": [
      {
        "q": "記録はどこに保存されますか？",
        "a": [
          "畑・畝・作付け・きろく帳は、利用中の端末のブラウザ内に保存します。アカウントによるクラウド保存や自動同期はありません。",
          "別の端末・ブラウザや別の公開先URLでは、同じ保存内容が表示されるとは限りません。公開サイトと確認用ページの保存先も別です。",
          "記録をサーバーへ同期する処理はありませんが、公開ページはアクセス解析や表示に必要な外部サービスへ通信します。「一切外部通信しない」アプリではありません。"
        ]
      },
      {
        "q": "バックアップはどうやって取りますか？",
        "a": [
          "画面の「⋯」メニューから「バックアップ」を選ぶと、畑と記録をまとめたJSONファイルが保存されます。保存先は端末・ブラウザによって異なります。",
          "機種変更、ブラウザのサイトデータ消去、畑の削除、大きな変更の前に保存してください。ファイルを別の安全な場所にも保管すると、端末が使えなくなった場合に役立ちます。"
        ]
      },
      {
        "q": "スマホとPCで共有・機種変更できますか？",
        "a": [
          {
            "type": "ol",
            "items": [
              "元の端末で「バックアップ」を保存する",
              "そのファイルを新しい端末へ渡す",
              "新しい端末のハタケボで「⋯」→「データを読込む」を選ぶ",
              "保存したJSONファイルを指定する"
            ]
          },
          "自動同期ではないので、片方を編集してももう片方には反映されません。",
          {
            "type": "warn",
            "text": "読み込みは追加・結合ではなく、そのブラウザにある畑と記録の置き換えです。読み込み先の記録も必要な場合は、先にそちらをバックアップしてください。"
          }
        ]
      },
      {
        "q": "記録が消えた・初期画面になりました",
        "a": [
          "まず、以前と同じ端末・ブラウザ・公開先URLで開いているか確認してください。プライベートブラウズやサイトデータの消去により、保存内容が残らないことがあります。",
          "バックアップがある場合は「データを読込む」で復元できます。元のブラウザにもバックアップにも記録がない場合、ハタケボ側のサーバーから復元することはできません。"
        ]
      },
      {
        "q": "「データを保存できません」と表示されます",
        "a": [
  ...
  },
  {
    ...
    "items": [
      ...
      },
      {
        "q": "文字が小さくて読みにくいです",
        "a": [
          "「⋯」メニューの「文字の大きさ」から「大きめ」「特大」を選べます。選んだ大きさは同じブラウザで記憶されます。",
          "大きな畑では、地図の表示倍率も調整できます。"
        ]
      },
      {
        "q": "紙に印刷したりPDFで保存したりできますか？",
        "a": [
          "「⋯」→「印刷する」から畑の図と作付け一覧を印刷できます。きろく帳に残した状態も印刷できます。",
          "PDF保存はブラウザや端末の印刷画面に用意されている場合に利用できます。用紙への収まりや色は印刷プレビューで確認してください。"
        ]
      },
      {
        "q": "オフラインでも使えますか？画面が表示されません",
        "a": [
          "ページの読み込みにはインターネット接続が必要です。表示用の部品を外部から読み込むため、HTMLだけを保存しても完全なオフライン利用にはなりません。",
          "開いた後に通信が切れても一部の操作が続けられる場合はありますが、再読み込みや再起動後の利用は保証していません。画面が出ない場合は接続を確認してください。画像が出ない場合は、サイトの更新ファイルが揃っていない可能性もあります。"
        ]
      },
      {
        "q": "不具合を問い合わせるときは何を伝えればよいですか？",
        "a": [
          "下の「お問い合わせ」から、使っている端末・ブラウザ、開いているURL、行った操作、画面に出たメッセージをお知らせください。スクリーンショットがあると状況を確認しやすくなります。",
          "ブラウザごとの動作保証はしていません。個人情報や畑の詳しい所在地など、問い合わせに不要な情報は送らないでください。"
        ]
      }
    ]
  }
    ...
    <svg viewBox={"0 0 "+SVG_W+" "+SVG_H} width={SVG_W} height={SVG_H} style={{display:"block"}}>
      ...
      {farm.grid.flatMap(function(row,r){
        return row.map(function(active,c){
          ...
          return <LandmarkImage key={"lmI"+r+"-"+c} item={it} x={cx2} y={cy2} size={Math.min(miniCS*0.6,18)}/>;
        });
      }).filter(Boolean)}
      {farm.grid.flatMap(function(row,r){
        var lines=[];
      ...
      {/* 外周目印 */}
      {padT>0&&Array.from({length:farm.cols},function(_,c){var it=lm["top-"+c];if(!it)return null;var cx2=padL+c*miniCS+miniCS/2;return(<g key={"lmT"+c}><LandmarkImage item={it} x={cx2} y={padT/2} size={LF}/>{LLF>5&&<text x={cx2} y={padT/2+LF*0.62} textAnchor="middle" fontSize={LLF} fill={C.inkFaint} fontFamily={SERIF}>{it.memo||it.label}</text>}</g>);}).filter(Boolean)}
      {padB>0&&Array.from({length:farm.cols},function(_,c){var it=lm["bot-"+c];if(!it)return null;var cx2=padL+c*miniCS+miniCS/2,cy2=padT+H+padB/2;return(<g key={"lmB"+c}><LandmarkImage item={it} x={cx2} y={cy2} size={LF}/>{LLF>5&&<text x={cx2} y={cy2+LF*0.62} textAnchor="middle" fontSize={LLF} fill={C.inkFaint} fontFamily={SERIF}>{it.memo||it.label}</text>}</g>);}).filter(Boolean)}
      {padL>0&&Array.from({length:farm.rows},function(_,r){var it=lm["lft-"+r];if(!it)return null;var cy2=padT+r*miniCS+miniCS/2;return(<g key={"lmL"+r}><LandmarkImage item={it} x={padL/2} y={cy2} size={LF}/>{LLF>5&&<text x={padL/2} y={cy2+LF*0.62} textAnchor="middle" fontSize={LLF} fill={C.inkFaint} fontFamily={SERIF}>{it.memo||it.label}</text>}</g>);}).filter(Boolean)}
      {padR>0&&Array.from({length:farm.rows},function(_,r){var it=lm["rgt-"+r];if(!it)return null;var cx2=padL+W+padR/2,cy2=padT+r*miniCS+miniCS/2;return(<g key={"lmR"+r}><LandmarkImage item={it} x={cx2} y={cy2} size={LF}/>{LLF>5&&<text x={cx2} y={cy2+LF*0.62} textAnchor="middle" fontSize={LLF} fill={C.inkFaint} fontFamily={SERIF}>{it.memo||it.label}</text>}</g>);}).filter(Boolean)}
      {/* 区切り線 */}
  ...
  function buildSVG() {
    ...
    farm.grid.forEach(function(row,r){
      row.forEach(function(active,c){
        ...
        parts.push(landmarkMarkup(it,fs,cx2,cy2));
    ...
    ridgeList.forEach(function(ridge){
      ...
      if(vg) {
        ...
        if(sz > 22) {
          ...
          parts.push(vegetableMarkup(vg.id,Math.min(sz*0.45,22),rr.x+2,rr.y+2));
        }
      } else {
        /* 野菜未設定の畝名 */
    ...
    function lmCell(item, cx, cy) {
      parts.push(landmarkMarkup(item,LF,cx,cy-2));
    ...
    return (
      <svg viewBox={"0 0 "+SVG_W+" "+SVG_H} width={SVG_W} height={SVG_H} style={{display:"block",maxWidth:"100%"}}>
        ...
        {farm.grid.flatMap(function(row,r){ return row.map(function(active,c){
          ...
          return (<g key={"lmI"+r+c}><LandmarkImage item={it} x={cx2} y={cy2} size={fs}/><text x={cx2} y={cy2+fs*0.62} textAnchor="middle" fontSize={Math.min(CS2*0.18,8)} fill={C.inkFaint} fontFamily={SERIF}>{it.memo||it.label}</text></g>);
        }).filter(Boolean); })}
